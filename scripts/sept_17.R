...
##In crashes one row represents one specific crash that occurred in NYC
##In persons one row represents a recorded person involved with the crashes
# What variable appears in both? Does it do the same job in both?
##Collision_ID. In crashes it is the primary key, foreign key in persons
...
persons_core |> count(UNIQUE_ID) |> 
  filter(n>1)

persons_core |> count(COLLISION_ID) |> 
  filter(n>1) 
